...
- Use fixed private IP addresses
...
The private network gateway is messing up with the bastion public IPv4 configuration. The fastest way I found was to remove the default route to the router (it is not going to survive a reboot): 

```bash
ssh <bastion_ipv6_address> # it's not going to work with the IPv4
...
```
**NB:** Do not forget to adapt the bastion public IPv4 address in the ansible inventory

## Access Prometheus
Same as before, setup a SOCKS proxy on your browser and access Prometheus on http://10.0.0.20:9090
